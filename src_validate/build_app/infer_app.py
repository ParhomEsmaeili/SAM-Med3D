...
#Sample application adapted from the SAMMed3D repository for the the SegFM challenge. 
...
class InferApp:
    def __init__(self,
        ...
        self.pre_normalise_bool = True #This is a boolean which determines whether we pre-normalise the image before passing it to the algorithm.
...
############################################################################################################################
    ...
    def binary_prop_to_model(self, im_dict: dict, is_state: dict | None, init: bool):
        ...
        if init:
            ...
            if self.pre_normalise_bool:
                # #Now we will clamp the image's voxel intensity distribution was not provided in their script, but in the same vein as the 
                ...
                if self.dataset_info['task_channels'][0] == "CT":
                    if self.app_params['checkpoint_name'] == 'sam_med3d_turbo_cvpr_coreset.pth':
                        ...
                        #     im_dict['metatensor'].array, self.clip_lower_bound
                        # ), np.percentile(
                        #     im_dict['metatensor'].array, self.clip_upper_bound) 
                    ...
                    try:
                        ...
                        if self.app_params['checkpoint_name'] == 'sam_med3d_turbo_cvpr_coreset.pth':
                            ...
                            # normalisation with z-score normalisation on the foreground (>0) voxels.
            ...
            #The original implementation was using a resampling for every single callback, but for some images this would be an absurd
            ...
            #our hand is forced by their implementation to resample each callback..
    ...
    #these applications so that they do not interfere! 
    ...
    def random_sample_next_click(prev_mask, gt_mask):
        ...
        """
        #This strategy would seem a bit confusing honestly. Its very very lossy. Given that they only forward propagate the foreground prompts,
        #yet here, they allow for the capability for sampling from the background prompts as well. 
    ...
    def sam_model_infer(
        ...
        with torch.no_grad(): #might be redundant.
            ...
            low_res_masks, _ = self.model.mask_decoder(
                ...
                #which had pre-set this to False.
            )
