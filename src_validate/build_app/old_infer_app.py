...
class InferApp:
    ...
    def binary_sw_inference(self, subject):
        ...
        #Implementing an image-level intensity normalisation in-line with the approach that the SAM-Med3D authors take in their inference script (likely suboptimal).
        intensity_transforms = tio.Compose([tio.Clamp(-1000,1000), tio.ZNormalization(masking_method=lambda x: x > 0)]) if self.dataset_info['task_channels'][0].title() == 'CT' else tio.Compose([tio.ZNormalization(masking_method=lambda x: x > 0)])
    ...
    def process_output(self, request_subj, input_subj, output_subj):
        ...
        affine = torch.from_numpy(copy.deepcopy(request_subj['image'].affine)).to(dtype=torch.float64)
